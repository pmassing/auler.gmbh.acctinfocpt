...
-- DROP FUNCTION pat_accountcourse(character varying, character varying, date, date);

CREATE OR REPLACE FUNCTION pat_accountcourse(accountfrom character varying, accountto character varying, datefrom date, dateto date)
...
BEGIN
    ...
    SELECT INTO currentBalance COALESCE(sum(amtacctdr),0.00) - COALESCE(sum(amtacctcr),0.00) from  pat_facourse fac 
                    where account_value=accountfrom and account_value=accountto and dateacct<datefrom;

    SELECT INTO count count(fact_acct_id) from  pat_facourse fac 
                    where account_value=accountfrom and account_value=accountto and dateacct>=datefrom and dateacct<=dateto;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION pat_accountcourse(character varying, character varying, date, date)
  OWNER TO adempiere;
